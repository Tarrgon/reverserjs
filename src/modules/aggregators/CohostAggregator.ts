...
import CohostScraper from "../customScrapers/CohostScraper"

class CohostAggregator implements Aggregator {
  index: number = 48
  manager: AggregationManager

  host: string = "cohost"
  displayName: string = "cohost"
  homepage: string = "https://cohost.org/"

  galleryTemplates: RegExp[] = [
    `cohost\\.org\\/${Globals.siteArtistIdentifier}`,
  ...
  submissionTemplate: string = "https://cohost.org/{siteArtistIdentifier}/post/{siteSubmissionIdentifier}"
  ...
  private async _internalProcess(artistUrl: ArtistURL, latestDate: Date): Promise<boolean> {
    ...
    try {
      for await (let media of CohostScraper.getMedia(artistUrl)) {
...
export default CohostAggregator