...
import ToyHouseScraper from "../customScrapers/ToyHouseScraper"

class ToyHouseAggregator implements Aggregator {
  index: number = 42
  manager: AggregationManager

  host: string = "toyhouse"
  displayName: string = "ToyHouse"
  homepage: string = "https://toyhou.se/"

  galleryTemplates: RegExp[] = [
    `toyhou\\.se\\/${Globals.siteArtistIdentifier}`,
    `toyhou\\.se\\/${Globals.siteArtistIdentifier}/art`
  ...
  submissionTemplate: string = "https://toyhou.se/{siteArtistIdentifier}/art#{siteSubmissionIdentifier}"
  ...
  private async _internalProcess(artistUrl: ArtistURL, latestDate: Date): Promise<boolean> {
    ...
    try {
      for await (let media of ToyHouseScraper.getMedia(artistUrl.urlIdentifier)) {
  ...
  async getApiIdentifier(urlIdentifier: string): Promise<string | null> {
    if (await ToyHouseScraper.verifyUrlIdentifier(urlIdentifier)) return urlIdentifier

    return null
  }
}

export default ToyHouseAggregator