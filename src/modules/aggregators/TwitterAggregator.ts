...
import TwitterScraper from "../customScrapers/TwitterScraper"
import ImageDownloader from "../ImageDownloader"

class TwitterAggregator implements Aggregator {
  index: number = 0
  manager: AggregationManager

  host: string = "x"
  displayName: string = "X"
  homepage: string = "https://x.com"

  galleryTemplates: RegExp[] = [
    `twitter\\.com\\/@?${Globals.siteArtistIdentifier}`,
    `x\\.com\\/@?${Globals.siteArtistIdentifier}`,
  ].map(r => new RegExp(`${Globals.prefix}${r}\\/?${Globals.remaining}\\??${Globals.remaining}#?${Globals.remaining}`))

  usernameIdentifierRegex: RegExp = /[a-zA-Z0-9_]{1,15}/

  submissionTemplate: string = "https://x.com/{siteArtistIdentifier}/status/{siteSubmissionIdentifier}"
  ...
  private async _internalProcess(artistUrl: ArtistURL, latestDate: Date): Promise<boolean> {
    ...
    try {
      for await (let media of TwitterScraper.getMedia(artistUrl)) {
...
export default TwitterAggregator