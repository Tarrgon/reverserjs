...
import PicatroScraper from "../customScrapers/PicartoScraper"

class PicartoAggregator implements Aggregator {
  index: number = 17
  manager: AggregationManager

  host: string = "picarto"
  displayName: string = "Picarto"
  homepage: string = "https://picarto.tv/"

  galleryTemplates: RegExp[] = [
    `picarto\\.tv\\/${Globals.siteArtistIdentifier}`,
    `picarto\\.tv\\/${Globals.siteArtistIdentifier}/gallery`
  ...
  submissionTemplate: string = "https://picarto.tv/{siteArtistIdentifier}/gallery/{album}/{siteSubmissionIdentifier}"
  ...
  private async _internalProcess(artistUrl: ArtistURL, latestDate: Date): Promise<boolean> {
    ...
    try {
      for await (let media of PicatroScraper.getMedia(artistUrl)) {
        ...
        for (let i = 0; i < media.mediaUrls.length; i++) {
          ...
          p.then(async (id) => {
            if (id) {
              let imageData = { ...id, source: this.submissionTemplate.replace("{siteArtistIdentifier}", artistUrl.urlIdentifier).replace("{siteSubmissionIdentifier}", media.id).replace("{album}", media.album), directLinkOffsite: url, offsiteId: `${media.id}_${i}` }
              let prom = Submission.create(artistUrl._id, imageData.offsiteId, imageData.source, imageData.md5, media.title, media.description, media.createdAt, imageData.width, imageData.height, imageData.fileSize, imageData.directLinkOffsite, imageData.extension, { tags: media.tags })
              promises.push(prom)
            }
          }).catch(console.error)
  ...
  async getApiIdentifier(urlIdentifier: string): Promise<string | null> {
    let apiIdentifier = await PicatroScraper.getApiIdentifier(urlIdentifier)

    return apiIdentifier
  }
}

export default PicartoAggregator