...
import NewgroundsScraper from "../customScrapers/NewgroundsScraper"

class NewgroundsAggregator implements Aggregator {
  index: number = 11
  manager: AggregationManager

  host: string = "newgrounds"
  displayName: string = "Newgrounds"
  homepage: string = "https://www.newgrounds.com"

  galleryTemplates: RegExp[] = [
    `${Globals.siteArtistIdentifier}\\.newgrounds\\.com`,
  ].map(r => new RegExp(`${Globals.prefix}${r}\\/?${Globals.remaining}\\??${Globals.remaining}#?${Globals.remaining}`))

  usernameIdentifierRegex: RegExp = /[a-zA-Z0-9~-]{1,20}/

  submissionTemplate: string = "https://www.newgrounds.com/art/view/{siteArtistIdentifier}/{siteSubmissionIdentifier}"
  ...
  private async _internalProcess(artistUrl: ArtistURL, latestDate: Date): Promise<boolean> {
    ...
    try {
      for await (let media of NewgroundsScraper.getMedia(artistUrl)) {
  ...
  async getApiIdentifier(urlIdentifier: string): Promise<string | null> {
    return await NewgroundsScraper.getApiIdentifier(urlIdentifier)
  }
}

export default NewgroundsAggregator