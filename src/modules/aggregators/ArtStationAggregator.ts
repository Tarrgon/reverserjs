...
import ArtStationScraper from "../customScrapers/ArtStationScraper"
...
class ArtStationAggregator implements Aggregator {
  ...
  host: string = "artstation"
  displayName: string = "ArtStation"
  homepage: string = "https://www.artstation.com"

  galleryTemplates: RegExp[] = [
    `artstation\\.com\\/artist\\/${Globals.siteArtistIdentifier}`,
    `artstation\\.com\\/${Globals.siteArtistIdentifier}`,
  ...
  private async _internalProcess(artistUrl: ArtistURL, latestDate: Date): Promise<boolean> {
    ...
    try {
      for await (let media of ArtStationScraper.getMedia(artistUrl)) {
  ...
  async getApiIdentifier(urlIdentifier: string): Promise<string | null> {
    return await ArtStationScraper.getApiIdentifier(urlIdentifier)
  }
}
