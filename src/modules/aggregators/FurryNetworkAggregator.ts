...
import FurryNetworkScraper from "../customScrapers/FurryNetworkScraper"

class FurryNetworkAggregator implements Aggregator {
  index: number = 34
  manager: AggregationManager

  host: string = "furrynetwork"
  displayName: string = "FurryNetwork"
  homepage: string = "https://furrynetwork.com"

  galleryTemplates: RegExp[] = [
    `furrynetwork\\.com\\/${Globals.siteArtistIdentifier}`,
    `beta\\.furrynetwork\\.com\\/${Globals.siteArtistIdentifier}`
  ].map(r => new RegExp(`${Globals.prefix}${r}\\/?${Globals.remaining}\\??${Globals.remaining}#?${Globals.remaining}`))

  usernameIdentifierRegex: RegExp = /[a-zA-Z0-9_-]{3,15}/

  submissionTemplate: string = "https://furrynetwork.com/artwork/{siteSubmissionIdentifier}"
  ...
  private async _internalProcess(artistUrl: ArtistURL, latestDate: Date): Promise<boolean> {
    ...
    try {
      for await (let media of FurryNetworkScraper.getMedia(artistUrl)) {
  ...
  async getApiIdentifier(urlIdentifier: string): Promise<string | null> {
    return await FurryNetworkScraper.getApiIdentifier(urlIdentifier)
  }
}

export default FurryNetworkAggregator